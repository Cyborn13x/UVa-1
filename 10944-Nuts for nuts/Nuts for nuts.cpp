...
int h,w,cnt,dp[16][1<<16];
int id[SZ][SZ],ds[SZ][SZ],adj[16][16];
char g[SZ][SZ];
...
void BFS(int x,int y)
{
      ...
      while(!qx.empty())
      {
      	...
      	rep(i,8)
      	{
      	      ...
      	      if(x2<0 || x2>=h || y2<0 || y2>=w || ds[x2][y2]!=INF) continue;

      	      ds[x2][y2]=ds[x1][y1]+1;
      	      qx.push(x2);
      	      qy.push(y2);
      	      if((v=id[x2][y2])!=-1) adj[u][v]=ds[x2][y2];
      	}
      }
