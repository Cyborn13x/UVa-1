...
#define repm(x,n) for(__typeof(n) x=(n);x>=0;x--)
#define FOR(x,m,n) for(__typeof(n) x=(m);x<=(n);x++)
#define FORI(x,m,n,i) for(__typeof(n) x=(m);x<=(n);x+=(i))
...
#define INFINI (__builtin_infd32())
...
int R,C,N,beeper;
int w[SZ][SZ],dp[SZ][1<<SZ];
int x[SZ],y[SZ];


int DP(int u,int bitmask)
{
      if(bitmask == (1<<N)-1) return w[u][0];
      if(dp[u][bitmask] != -1) return dp[u][bitmask];

      int ret=INF;

      repl(i,N) if(!(bitmask&(1<<i))) ret = min(ret,w[u][i] + DP(i,bitmask|(1<<i)));

      return dp[u][bitmask] = ret;
}
...
int main()
{
	...
	while(t--)
	{
		...
		repl(i,beeper) sii(x[i],y[i]);

		N=beeper+1;

		rep(i,N) rep(j,N) w[i][j] = abs(x[i]-x[j]) + abs(y[i]-y[j]);
